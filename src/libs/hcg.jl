# Hot carrier generation code
include("aux.jl")

function build_mask_hcg(E, Ep, fermi_eV, beta, hw, kappa, gph)
    # Prefactors in the hot carrier calculation which does not depend on the Hamiltonian
    # everything is in units of eV
    Nx = length(E)
    Ny = length(Ep)
    ...
    gi = getgi(E,gph,kappa,fermi_eV)
    gj = getgi(Ep,gph,kappa,fermi_eV)
    ...
    for i=1:Nx 
        Ei = E[i]
        fi = 1.0/(1.0 + exp(beta*(Ei-fermi_eV)))

        for j=1:Ny 
            Ej = Ep[j]
            fj = 1.0/(1.0 + exp(beta*(Ej-fermi_eV)))
...
function sum_hcg(phi,Ei,Ej,Nx,Ny,fermi_Ha, hw, beta, kappa, gph, debug)
    # Use optical matrix in energy space to build the hot carrier generation rate
    ...
    phi1 = phi_itp(xs, ys)
    mask_el, mask_ho = build_mask_hcg(xs,ys, fermi_eV, beta, hw, kappa, gph)
...
function hcg_auto(mumn, A, B, NE, fermi_Ha, freq, beta, kappa, gph)
...
function hcg_conv(mumn, A, B, NE, fermi_Ha, freq, beta, kappa, gph, print_conv)
    ...
    hcg1 = sum_hcg(opt1, Ei,  Ej, NE, NEp,  fermi_Ha, freq, beta, kappa, gph, write)
    hcg2 = sum_hcg(opt2, Ei1, Ej, NE, NEp,  fermi_Ha, freq, beta, kappa, gph, write)
    hcg3 = sum_hcg(opt1, Ei,  Ej, NE, NEp1, fermi_Ha, freq, beta, kappa, gph, write)
    ...
    hcg4 = sum_hcg(opt2, Ei, Ej, NE, NEp, fermi_Ha, freq, beta, kappa, gph, write)
    hcg5 = sum_hcg(opt3, Ei, Ej, NE, NEp, fermi_Ha, freq, beta, kappa, gph, write)
    hcg6 = sum_hcg(opt4, Ei, Ej, NE, NEp, fermi_Ha, freq, beta, kappa, gph, write)
    ...
    hcg7 = sum_hcg(opt1, Ei,  Ej, NE, NEp,  fermi_Ha, freq, beta, kappa, gph*2, write)
    hcg8 = sum_hcg(opt1, Ei,  Ej, NE, NEp,  fermi_Ha, freq, beta/2, kappa, gph, write)
