...
function build_mask(E, Ep, fermi, beta, hw, Gammaf, Gamma)
    ...
    df = fermi_dirac(E,beta,fermi) - fermi_dirac(Ep,beta,fermi)

    return -4*df/g*lorentz
end


function sum_dist(phi, Ei, Ej, Nx, Ny, fermi_eV, hw, beta, Gammaf, Gamma, write)
